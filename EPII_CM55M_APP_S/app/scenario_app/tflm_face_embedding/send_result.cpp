...
/*
 * Face Recognition JSON output functions
 * Used by Web Debug Tool for real-time preview and face enrollment
 */

std::string embedding_2_json_str(const float* embedding, int dim) {
    if (!embedding || dim <= 0) {
        return "\"embedding\": \"\", \"embedding_format\": \"none\", \"embedding_dim\": 0";
    }

    /* Debug: check embedding pointer and values (after normalization) */
    DBG_INFO("[emb2json] ptr=0x%08X, dim=%d, first3(x10000)=%d,%d,%d\n",
            (uint32_t)embedding, dim,
            (int)(embedding[0]*10000), (int)(embedding[1]*10000), (int)(embedding[2]*10000));

    /* Debug: print raw bytes of first float to verify memory */
#if DEBUG_VERBOSE >= 1
    const unsigned char* raw = (const unsigned char*)embedding;
    DBG_INFO("[emb2json] raw bytes[0-7]: %02X %02X %02X %02X | %02X %02X %02X %02X\n",
            raw[0], raw[1], raw[2], raw[3], raw[4], raw[5], raw[6], raw[7]);
#endif

    /*
     * Binary Base64 encoding for embedding (much faster than JSON array):
     * - 128/512 floats * 4 bytes = 512/2048 bytes binary
     * - Base64 encoded = ~683/2732 bytes (vs ~1.3/5KB for JSON array)
     * - Much faster to encode (no float-to-string conversion)
     *
     * Format: "embedding": "<base64>", "embedding_format": "float32_base64", "embedding_dim": N
     */
    const int binary_size = dim * sizeof(float);
    const int base64_size = ((binary_size + 2) / 3) * 4 + 1;

    /* Static buffer sized for max embedding (512D) */
    constexpr int MAX_BINARY_SIZE = 512 * sizeof(float);  /* 2048 bytes max */
    constexpr int MAX_BASE64_SIZE = ((MAX_BINARY_SIZE + 2) / 3) * 4 + 1;  /* ~2732 bytes */
    constexpr int TOTAL_BUF_SIZE = MAX_BASE64_SIZE + 80;

    static char base64_buf[MAX_BASE64_SIZE];
    static char result_buf[TOTAL_BUF_SIZE];

    /* Encode embedding as Base64 using actual dimension */
    el_base64_encode((const unsigned char*)embedding, binary_size, base64_buf);

    /* Debug: check base64 output first chars */
    DBG_INFO("[emb2json] base64 first 20: %.20s\n", base64_buf);

    snprintf(result_buf, TOTAL_BUF_SIZE,
             "\"embedding\": \"%s\", \"embedding_format\": \"float32_base64\", \"embedding_dim\": %d",
             base64_buf, dim);

    return std::string(result_buf);
}

std::string landmarks_2_json_str(const float* landmarks, int num_points) {
    if (!landmarks || num_points <= 0) {
        return "\"landmarks\": []";
    }

    std::string ss = "\"landmarks\": [";
    for (int i = 0; i < num_points; i++) {
        if (i > 0) ss += ", ";
        char buf[32];
        /* Use integer math since snprintf %.f may not work on this platform */
        int x = (int)(landmarks[i*2] * 10);
        int y = (int)(landmarks[i*2+1] * 10);
...
void send_face_recognition_json(
    ...
    /* Image (optional) */
    if (jpeg_img && jpeg_img->data && jpeg_img->size > 0) {
        data_str += img_2_json_str(jpeg_img);
        data_str += ", ";
        data_str += img_res_2_json_str(jpeg_img);
    } else {
        data_str += "\"image\": \"\", \"resolution\": [0, 0]";
    }

    /* Faces array */
    data_str += ", \"faces\": [";

    /* Output face data if face_box exists (embedding is optional) */
    if (face_box) {
        data_str += "{";

        /* Bounding box */
        char bbox_buf[64];
        snprintf(bbox_buf, sizeof(bbox_buf), "\"bbox\": [%d, %d, %d, %d]",
                 face_box->x, face_box->y, face_box->w, face_box->h);
        data_str += bbox_buf;

        /* Confidence - use integer math since snprintf %.f may not work */
        char conf_buf[32];
        int conf_int = (int)(confidence * 1000);  /* e.g. 0.695 -> 695 */
        snprintf(conf_buf, sizeof(conf_buf), ", \"confidence\": %d.%03d",
                 conf_int / 1000, conf_int % 1000);
        data_str += conf_buf;

        /* Landmarks (optional) */
        if (landmarks) {
            data_str += ", ";
            data_str += landmarks_2_json_str(landmarks, 5);
        } else {
            data_str += ", \"landmarks\": []";
        }
        ...
        if (embedding && embedding_dim > 0) {
            data_str += ", ";
            data_str += embedding_2_json_str(embedding, embedding_dim);
        } else {
            data_str += ", \"embedding\": []";
        }

        /* Aligned face image - temporarily disabled for testing */
        (void)aligned_face;
        (void)aligned_w;
        (void)aligned_h;
    ...
    /* Build final JSON */
    const auto& ss = concat_strings(
        "\r{\"type\": 1, \"name\": \"FACE_RESULT\", \"code\": 0, \"data\": {",
        data_str,
        "}}\n"
    );

    /* Debug: print JSON size */
    DBG_INFO("[JSON] Sending %d bytes, has_face=%d\n", ss.size(), face_box ? 1 : 0);
