/*
 * tflm_face_embedding.c
 *
 *  Created on: Dec 11, 2024
 *      Author: Face Embedding App (MobileFaceNet 128D)
 *
 * This application performs:
 * 1. Face detection using SCRFD (with 5-point landmarks)
 * 2. Face alignment using similarity transform
 * 3. 128D embedding extraction using MobileFaceNet
 * 4. Embedding transmission via UART
 */
...
static void dp_app_cv_fd_fm_eventhdl_cb(EVT_INDEX_E event)
{
	...
	else if( g_trans_type == 1 || g_trans_type == 2)// transfer type is (SPI) or (UART & SPI)
	{
		...
		#if FRAME_CHECK_DEBUG
				hx_drv_spi_mst_protocol_write_sp((uint32_t)&embedding_result, sizeof(face_embedding_msg_t), DATA_TYPE_META_FM_WITH_FPS_DATA);
		#endif
	}
#else
	#if TOTAL_STEP_TICK
		...
		/* Retrigger capture for next frame - THIS IS CRITICAL! */
		sensordplib_retrigger_capture();
	}
...
void app_start_state(APP_STATE_E state)
{
	...
	if( g_trans_type == 0 || g_trans_type == 2)// transfer type is (UART) or (UART & SPI)
	{
		...
		{
			if(cisdp_dp_init(true, SENSORDPLIB_PATH_INT_INP_HW5X5_JPEG, dp_app_cv_fd_fm_eventhdl_cb, 4, APP_DP_RES_RGB640x480_INP_SUBSAMPLE_2X) < 0)// RGB 320X240
			{
				xprintf("\r\nDATAPATH Init fail\r\n");
				APP_BLOCK_FUNC();
			}
...
int app_main(void) {
...
#if 0  // Disable watchdog for debugging
	...
	/* Use linker symbol to avoid memory overlap with static allocations */
#ifdef __GNU__
	...
	//Face Detection and Embedding (MobileFaceNet 128D)
	xprintf("Face Detection and Embedding (MobileFaceNet 128D)\n");
	cv_face_embedding_init(true, true, FACE_DETECT_FLASH_ADDR, FACE_EMBEDDING_FLASH_ADDR);
